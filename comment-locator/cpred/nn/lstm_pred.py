...
class LSTM_PRED(object):
  ...
  def __init__(self, config):
    ...
    with tf.name_scope("Input"):
      self.inputs = tf.placeholder(tf.int32, shape=(None, seq_len, sent_len), name="inputs")
      self.input_wts = tf.placeholder(tf.float32, shape=(None, seq_len, None), name="input_wts") 
      batch_size = tf.shape(self.inputs, out_type=tf.int32)[0]
      self.targets = tf.placeholder(tf.int32, shape=(None, seq_len), name="labels")
      self.target_weights = tf.placeholder(tf.float32, shape=(None, seq_len), name="tgt_wts")
    ...
    with tf.name_scope("Embeddings"):
      ...
      if embed_type == data_helper.AVG_WEMBED or embed_type == data_helper.AVG_CEMBED:
        self.input_wts_resh = tf.tile(tf.reshape(self.input_wts, [batch_size, seq_len, sent_len, 1]), [1, 1, 1, embed_size])
    ...
    with tf.name_scope("RNN"):
      ...
      lstm_layers = tf.contrib.rnn.MultiRNNCell([lstm_cell_with_dropout() for _ in range(lstm_num_layers)], state_is_tuple=True)
      self.reset_state = lstm_layers.zero_state(batch_size, data_type())
      self.outputs, self.next_state = tf.nn.dynamic_rnn(lstm_layers, self.input_embeddings, time_major=False, initial_state=self.reset_state)
      
    with tf.name_scope("cost"):
      self.output = tf.reshape(tf.concat(axis=0, values=self.outputs), [-1, lstm_hidden_size])
      ...
      self.logits = tf.reshape(tf.nn.bias_add(tf.tensordot(self.outputs, self.op_w, axes=[[2], [0]]), self.op_b), [-1, nlabels])
      # self.logits = tf.add(tf.matmul(self.output, self.op_w), self.op_b)
...
class Config(object):
  ...
  def __init__(self, inits, lr, mgrad, seq_len, sent_len, lstm_layers, lstm_hsize, mepoch, kp, decay, bsize, tbsize, labsize, vocab_size, sembed_type, sembed_size, vnimp, ckpt_epochs, model_type, use_pretraining):
